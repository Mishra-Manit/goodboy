...
import { listRepoNames } from "../../src/shared/repos.js";
...
async function run(): Promise<void> {
  const repoNames = listRepoNames();
  const startedAt = performance.now();
  const intent = await classifyMessageWithModel(MESSAGE, repoNames, MODEL);
  const latencyMs = performance.now() - startedAt;

  console.log("=== TELEGRAM INTENT CLASSIFIER LATENCY ===");
  console.log(`model      : ${MODEL}`);
  console.log(`repo count : ${repoNames.length}`);
